...
def build_standalone_analytics():
    ...
    for name in STANDALONE:
        ...
        if new != doc:
            ...
            print("updated", name)
        elif MARK_OPEN not in new and "</body>" not in doc:
            # A page with no </body> silently received nothing: the replace found
            # no target, returned the document unchanged, and the loop reported
            # success. Say so instead.
            print(f"WARNING {name}: no </body> and no marker - analytics not added")
