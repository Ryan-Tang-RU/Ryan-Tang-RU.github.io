...
The site is served at `https://www.ruixiangtang.net/`. The `CNAME` file holds that domain and
`domain:` in `_data/site.yml` matches it, which is what the canonical tags, `og:url`, `sitemap.xml`
and `robots.txt` are built from — change one and change the other. DNS lives at Squarespace: `www`
is a CNAME to `ryan-tang-ru.github.io`, and the bare `ruixiangtang.net` forwards to `www` through a
Squarespace forwarding rule. `ryan-tang-ru.github.io` now redirects to the custom domain, so older
links still resolve.
